...
class JungleSelect extends Component {
  ...
  selectHighlightedItem(e) {
    ...
    const { highlighted, filter } = this.state
  ...
  onClear(e) {
    ...
    const { filter, listOpened, focused } = this.state
  ...
  filteredItems(items) {
    const { filterItem, searchableAttributes, limit } = this.props
  ...
  renderList() {
    const { groups, items, renderGroup, renderShowAll, limit } = this.props
    const { showAll } = this.state
    let counter = -1
    if (groups) {
      return groups.map((group, groupIndex) =>
        ...
        >
          <div className='jungle-select-group-title'>{renderGroup(group)}</div>
          <div>
            {this.filteredItems(this.itemsForGroup(group)).map((item, itemIndex) => {
              counter = counter + 1
  ...
  render() {
    ...
    const {
      placeholder, searchable, listWrapper, classList, clearable, mode
    } = this.props
    const { filter, listOpened, focused } = this.state
    const selected = this.selectedItems()
